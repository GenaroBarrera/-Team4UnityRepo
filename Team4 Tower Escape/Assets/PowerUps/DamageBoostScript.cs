...
public class DamageBoostScript : MonoBehaviour {
    ...
    IEnumerator Pickup(Collider player) //we call our param palyer, since at this point we konw that the player has picked up the power up
    {
        ...
        PlayerShooting playerShooting = player.GetComponentInChildren<PlayerShooting>(); //Get a reference to our PlayerMovement script and store it as playerMovement(Note: player.)
        playerShooting.damagePerShot *= multiplier; //now we can modify this variable in the PlayerMovement script using playerMovement
        ...
        playerShooting.damagePerShot /= multiplier; //reset the player's speed
